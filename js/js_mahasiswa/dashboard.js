(() => {
  if (window.__MHS_DASH__) return;
  window.__MHS_DASH__ = true;
  ...
  window.addEventListener("load", () => {
    const loginQ = (app.dataset.login || "").trim();
  ...
  const flashType = (app.dataset.toastType || "").trim();
  const flashMsg = (app.dataset.toastMsg || "").trim();
  const loginQ = (app.dataset.login || "").trim();
  ...
  if (flashMsg) {
    ...
    showToast("success", "Berhasil", "Login berhasil. Selamat datang!", 2600);
    try {
      const u = new URL(window.location.href);
      u.searchParams.delete("login");
      window.history.replaceState({}, "", u.toString());
  ...
  const apiUrl = dashboardPage?.dataset?.api || "";
  const liveBlockedMsg = (dashboardPage?.dataset?.blockedMsg || blockedMsg || "").trim();
